							    	...
							    	$prodIds = $HTTP_GET_VARS['prodIds'];
										...
										for ($i=0; $i < count($prodIds); $i++) 
										{
										   ...
										   mysql_query($sql) or error(__LINE__,__FILE__,"Konnte Produkt nicht ändern.",mysql_error());
												...
												$sql = "SELECT * 
																FROM produktpreise 
																WHERE produkt_id = ".mysql_escape_string($prodIds[$i])." 
...
//					       ORDER BY p.name") or error(__LINE__,__FILE__,"Konnte Produkte nich aus DB laden..",mysql_error());
	...
	?>
			...
			<?php 				//preise werden eingefügt
					$sql = "SELECT preis 
										FROM produktpreise 
										WHERE produkt_id = ".mysql_escape_string($row['prodId'])." 
										AND (zeitende >= NOW() OR ISNULL(zeitende))";
				  $result2 =  mysql_query($sql) or error(__LINE__,__FILE__,"Konnte Preise nich aus DB laden..",mysql_error());
	        ...
	        <tr>
						 ...
						 <td valign="top" align="middle"><input type="text" name="notiz_<?PHP echo $row['prodId']; ?>" value="<?PHP echo $row['notiz']; ?>"></td>
						    <?PHP
								   $result2 =  mysql_query("SELECT * FROM produktpreise WHERE produkt_id = ".mysql_escape_string($row['prodId'])." AND (zeitende >= NOW() OR ISNULL(zeitende));") or error(__LINE__,__FILE__,"Konnte Preise nich aus DB laden..",mysql_error());
									 if (mysql_num_rows($result2) > 1)
