...
open_form( 'small_form', '', '', array( 'action' => 'save' ) );
  open_fieldset( 'small_form', "style='width:470px;'", ( $lieferanten_id ? 'Stammdaten Lieferant' : 'Neuer Lieferant' ) );
    ...
    open_table('small_form');
        open_td( 'label', '', 'Name:' );
        open_td( 'kbd', '', text_view( $name, 50, '', ( $editable ? 'name' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Adresse:' );
        open_td( 'kbd', '', text_view( $adresse, 50, '', ( $editable ? 'adresse' : false ) ) );
      open_tr();
        open_td( 'label', '', 'AnsprechpartnerIn:' );
        open_td( 'kbd', '', text_view( $ansprechpartner, 50, '', ( $editable ? 'ansprechpartner' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Telefonnummer:' );
        open_td( 'kbd', '', text_view( $telefon, 50, '', ( $editable ? 'telefon' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Faxnummer:' );
        open_td( 'kbd', '', text_view( $fax, 50, '', ( $editable ? 'fax' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Email:' );
        open_td( 'kbd', '', text_view( $mail, 50, '', ( $editable ? 'mail' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Liefertage:' );
        open_td( 'kbd', '', text_view( $liefertage, 50, '', ( $editable ? 'liefertage' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Bestellmodalit&auml;ten:' );
        open_td( 'kbd', '', text_view( $bestellmodalitaeten, 50, '', ( $editable ? 'bestellmodalitaeten' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Kundennummer:' );
        open_td( 'kbd', '', text_view( $kundennummer, 50, '', ( $editable ? 'kundennummer' : false ) ) );
      open_tr();
        open_td( 'label', '', 'Webadresse:' );
        open_td( 'kbd', '', text_view( $url, 50, '', ( $editable ? 'url' : false ) ) );
      open_tr();
        open_td( 'right', "colspan='2'" );
          ...
          open_span( 'qquad' );
          if( $editable )
            submission_button( $lieferanten_id ? 'Speichern' : 'Einfügen' );
          else
            close_button();
          close_span();
    close_table();
