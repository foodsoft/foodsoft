    ...
    foreach( sql_gruppe_mitglieder( $gruppen_id ) as $row ) {
      ...
      open_table('form');
        ...
        if ($editable) {
          open_tr();
            open_td( '', '', 'Slogan: ' );
            open_td( '', '', string_view( $row['slogan'], 80, $editable ? "slogan_$id" : false ) );
          open_tr();
            open_td( '', '', 'URL: ' );
            open_td( '', '', string_view( $row['url'], 255, $editable ? "url_$id" : false ) );
          open_tr();
            open_td( '', '', 'Bild: ' );
            open_td( '', '');
              open_tag('input', '', "name='avatar_$id' type='file' size='40' maxlength='256000' accept='image/jpg,image/png'", '');
              open_tag('input', '', "name='avatar_delete_$id' type='checkbox' value='delete'", 'löschen');
        }
        open_tr();
