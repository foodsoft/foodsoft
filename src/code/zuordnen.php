      ...
      case 'katalog':
        if ($cond) {
          $joins = array_merge($joins, need_joins_array($using, [
            ...
            . 'AND lieferantenkatalog.artikelnummer = produkte.artikelnummer)', 'hersteller_acro' => 'LEFT OUTER JOIN catalogue_acronyms as hersteller_acro '
            . 'ON (hersteller_acro.context = "hst" '
            . 'AND hersteller_acro.acronym = lieferantenkatalog.hersteller COLLATE utf8mb3_unicode_ci)', 'verband_acro' => 'LEFT OUTER JOIN catalogue_acronyms as verband_acro '
            . 'ON (verband_acro.context = "vbd" '
            . 'AND verband_acro.acronym = lieferantenkatalog.verband COLLATE utf8mb3_unicode_ci)', 'herkunft_acro' => 'LEFT OUTER JOIN catalogue_acronyms as herkunft_acro '
            . 'ON (herkunft_acro.context = "hrk" '
            . 'AND herkunft_acro.acronym = lieferantenkatalog.herkunft COLLATE utf8mb3_unicode_ci)'
          ]));
          alias_columns($selects, 'lieferantenkatalog', 'katalog', [
            'ean_einzeln',
            'bemerkung',
            'hersteller',
            'verband',
 ...
 *   - 'katalog' (optional)
 *     Set to TRUE in order to query catalogue data alongside
 * @param string $orderby
 * @return string
 ...
 *   result records
 */
function sql_bestellung_produkte( array $keys, string $orderby = '' ) {
...
function sql_bestellungen_soll_gruppe( $gruppen_id, $bestell_id = 0 ) {
  ...
  $query = "
    SELECT gesamtbestellungen.id as gesamtbestellung_id
         ...
         , gesamtbestellungen.rechnungsstatus AS gesamtbestellung_status
...
// select_produktpreise_konsistenztest:
//  - alle Zeitintervalle bis auf das letzte müssen abgeschlossen sein
//  - Intervalle dürfen nicht überlappen 
...
function sql_produktpreise_konsistenztest($lieferanten_id = false, $produkt_id = false) {
  ...
  foreach (
    mysql2array(
      doSql(
        select_produktpreise_konsistenztest($lieferanten_id, $produkt_id)
      )
    ) as $problem
  ) {
